// Procedural 8-bit NES Sound Generator using Web Audio API
...
class SoundEffectsManager {
  ...
  public playPunch(): void {
    ...
    // NES Kung-Fu punch: Square wave rapid drop + filtered noise
    const osc = ctx.createOscillator();
    const gain = ctx.createGain();
    osc.type = 'triangle';
  ...
  public playKick(): void {
    ...
    this.playNoiseBurst(0.12, 0.2);
  }

  public playEnemyKnockout(): void {
    if (!this.enabled) return;
    const ctx = this.getContext();
    if (!ctx) return;

    // Fast ascending sweep + impact
    const osc = ctx.createOscillator();
    const gain = ctx.createGain();
    osc.type = 'sawtooth';
  ...
  public playFanfare(): void {
    ...
    notes.forEach((freq, idx) => {
      ...
      const duration = idx === notes.length - 1 ? 0.4 : 0.1;
  ...
  public playHardMode(): void {
    if (!this.enabled) return;
    const ctx = this.getContext();
    if (!ctx) return;

    // Deep ominous sweep down
    const osc = ctx.createOscillator();
    const gain = ctx.createGain();
    osc.type = 'sawtooth';
